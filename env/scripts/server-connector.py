    ...
    if len(local_bindings) > 0 and 0:
        ...
        for zonename in local_bindings:
            ...
            for share in zone[KEY_SHARE]:
                ...
                print("'%s' locally bind for zone: %s on (%s)" %
                      (share[KEY_SHARE], zonename, share[KEY_MOUNT_POINT]))
...
class KZoneConnectorChroot(KZoneConnector):
    ...
    def init_chroot(self, bind_dirs, rbind=False, checkRbindDblMount=False):
        ...
        for bind_dir in bind_dirs:
            ...
            if rbind and checkRbindDblMount and target_dir + ' ' in mounts:
                break
...
def main():
    ...
    if os.getuid() != 0:
        # Before privilege elevation (on legacy servers, root can't write or even check in user HOME)
        create_nabla_ssh_key_if_needed()
    ...
    if args.verbose_mode:
        ...
        if isLinux():
            ...
            print('Distribution:\t%s "%s" (%s)' %
                  (distr[0], distr[2], distr[1]))
