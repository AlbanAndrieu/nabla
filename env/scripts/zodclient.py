...
def read_check_update_resp(resp_data):
    if resp_data['zodclient_py'] != VERSION:
        print >> sys.stderr, "WARNING: zodclient('" + VERSION + \
            "') needs to be updated to '" + resp_data['zodclient_py'] + "'"
...
def parseArgs(arguments=None):
    parser = argparse.ArgumentParser(
        ...
        # specified so that it still displays the right name when launched as a module
        prog='zodclient',
    )
...
def mandatoryArg(arg, notEmpty=False, notZero=False):
    ...
    if isinstance(val, types.StringTypes) and re.match(r'^\s*$', val) and notEmpty == True:
        error('Error: --%s parameter must not be empty for action %s' %
              (arg, args.action))
    if isinstance(val, types.IntType) and str(val) == '0' and notZero == True:
        error('Error: --%s parameter must not be zero for action %s' %
              (arg, args.action))
    return val
